...
def RandomForest(_method, _N):
    ...
    for i in range(k):
        for train_index, test_index in kf.split(subtrain):
            ...
            print("accuracy: {0:.4f}, precision: {1:.4f}, recall: {2:.4f}, F1: {3:.4f}".format(score, precision, recall,
                                                                                               F1))
            _sum_recall += recall
    ...
    df.to_json(generatepath + method + str(N) + "RF.json")
    return df
